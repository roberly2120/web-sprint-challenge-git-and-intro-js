...
addArtist(artists, 'ryan', '1993-current', 'Web Design', 'American', 'I have a background in restaurants and real estate. I am attending BloomTech to bcome a Frontend Developer');
...
function artistByCountry(array, nationality){
  let artistsFromPlaces = [];

  for (let i = 0; i < array.length; i++) {
    if (array[i].nationality === nationality) {
      artistsFromPlaces.push(array[i].name);
    }
  }
  return artistsFromPlaces;
}
